...
%% 1.1.40
	...
	\begin{proof}
		\begin{itemize}[]
			\item $\afs (v) \df (i) \afs$ Sei $(U_i)_{i\in I}$ offene Überdeckung von $X$.
			Angenommen $\not\exists$ endliche Teilüberdeckung. Definiere induktiv Kugeln $K_n$ $n\in\N$, wie folgt
			Da $X$ präkompakt, gibt es zu $\varepsilon = 1$ endliche viele Kugeln $U_1(x_0,j)$ mit 
			$X \subset \cap_{j=0}^{m_1} U_1(x_{0,j}$.
			Dann gilt: Mindestens eine dieser Kugeln ist nicht durch endliche viele Mengen aus $(U_i)_{i\in I}$ überdeckbar.\par
			OBdA $U_1(x_{0,0})$, setze $x_0 := x_{0,0}$. \par
			Konstruiere so eine Folge $(x_n)$, so dass $U_{\frac{1}{2^n}}(x_1)$ nicht druch dendliche viele Mengen aus $(U_i)_{i\in I}$ überdeckt werden kann. \par
			...
			$\df (x_n)$ ist CF in $X$ $\df$ 
			$\exists  \hat{x} \in X$ ($X$ vollständig), so dass $\limes d(x_n,\hat{x}) = 0$\par
			Wegen $X = UU$ gilt $\exists i_0\in I: \hat{x}\in U_{i_0}$\par
			Weil $U_{i_0}$ offen: $\exists r>0$, so dass $U_r(\hat{x} \subset U_{i_0}$\par
			Sei  $n\in \N$, so dass $\frac{1}{2^n} < \frac{r}{2}$ und $d(\hat{x}, x_n) < \frac{r}{2}$\par
			$\df U_{\frac{1}{2}} (x_n) \subset U_r(\hat{x}) \subset U_{i_0}$\par
			Das ist ein Widerspruch dazu, dass $UU$ nicht durch endliche viele $U_i$ überdeckt werden kann.
		\end{itemize}
	\end{proof}
	...
	\begin{cor}
		$(X,d)$ metrischer Raum
			\begin{enumerate}[a)]
				...
				\item $M\subset X$, so dass jede Folge in M eine in M konvergente TF hat ("M folgenkomapkt")
					$\Leftrightarrow$ $M \subset X$ kompakt ("M Überdeckungskompakt")
	...
	\begin{definition}
		$(X,d)$ metrischer Raum. $M\subset X$ heißt \qmarks{relativ kompakt}, wenn $Mstrich$ kompakt.
	\end{definition}

	\begin{definition}
		$X$ vollständig, $M \subset X$ relativ kompakt. \par
			$\Leftrightarrow$ jede Folge in $M$ besitzt eine in $X$ konvergente TF.
	\end{definition}

	\begin{thm}
		$X$ metrischer Raum. $M,n \subset X$ seien relativ kompakt (bzw. präkompakt). Dann gilt 
			\begin{enumerate}
				\item $S\subset M$ $\df$ $S$ relativ kompakt (bzw. präkompakt)
				\item $M\cup N$ relativ kompakt (bzw präkomp.)
				\item $M,N$ präkompakt
	...
	\begin{proof}
		\begin{itemize}
			...
			\item d) $\df$ folgt aus c) \par
				$\Leftarrow$ Sei $M$ präkompakt $\forall \varepsilon > 0 \exists p\in\N, \{x_1,\dots,x_p\} \subset X$ mit $M \subset \cup_{j=1}^p U_{\frac{\varepsilon}{2}}(x_j)overline$. 
				Wegen $\overline{U_{\frac{\varepsilon}{2}}} = U_{{\varepsilon}}$ gilt
				$\overline{M} \subset \cup_{j=1}^p \overline{U_{\frac{\varepsilon}{2}}} \subset \cup_{j=1}^p U_{varepsilon}$
				$\df$ $\overline{M}$ präkompakt. Da $(\overline{M},d)$ vollständig ist $\overline{M}$ kompakt. (Satz 1.4.1) $\df$ $M$ relativ komapkt.

		\end{itemize}
	\end{proof}

%Der Heuser beschreibt den folgenden Satz folgendermaßen: 
...
\newcommand{\xt}{x_\theta}
	\begin{thm}
		TODO(eigentlich LEMMA!!) $(X,\norm{\cdot}$ normierter Raum, $E\subset X$ abgeschlossen UVR mit $E \not = X$, $\theta\in (0,1)$. \par
		Dann existiert ein $x_\theta \in X$ mit $\norm{\xt} = 1$ und $\norm{\xt - y} \geq \theta \; y\in E.$
	\end{thm}
	\begin{proof}
...
%		Für $y\in E$ gilt $\norm{\xt - y} - \norm{y - \frac{ }{}}$ = \norm{y + \frac{}{} - \frac{}{}} = \frac{}{} \norm{(\norm{x_0 - z}y + z) - x_0} \geq \frac{}{} \delta \geq \frac{eta}{\delta} \delta = \eta$
	\end{proof}	
	...
	\begin{proof}
		a) Folgt aus Heine-Borel\par
		$\Leftarrow$ \par
		$\Rightarrow$ Angenommen, $dim X = \infty$. Wähle $x_0 \in X$ mit $\norm{x_0} = 1$. Nach Lemma von Riesz, wähle $x_1 \in X$, so dass $\norm{x_1 - y} \geq \frac{1}{2} \forall spann\{x_0\}$. \par
		Konstruiere so Folge $(x_n)$ mit $\norm{x_n} = 1$ und $\norm{x_n - y} \geq \frac{1}{2} \forall y\in span\{x_0,\dots,x_{n-1}\} \forall n\in\N.$ \par
		$\df \norm{x_n - x_m} \geq \frac{1}{2} \forall n,m \in \N$  mit $n \not = m$. 
		$\df (x_n)$ hat keine konvergente TF. \par
		b) genauso.
	\end{proof}
	...
	(S2) $(x,y) = (y,z)$
	(S3) $(x,y) > 0 \forall x\in X\backslash{0}$.
	...
	\begin{bem}
		\begin{enumerate}
			...
			\item Für $x,y \in X$ gilt die Para.. 
				$\norm{x + y} ^2 + \norm{x - y}^2 = 2 \norm{x}^2 + 2\norm{y}^2$
