...
%TODO: Über das Beweiskästchen streiten wir nochmal ;) DS (<- Mein Namenskürzel)
	...
	\begin{proof}
		...
		\item Sei $\lambda \in \K,\;T\in\BS\df\;\norm{\lambda T}=\sup_{x\in U_1(0)} \norm{\lambda T(x)}_Y = |\lambda| \sup_{x\in U_1(0)}\norm{T(x)}_Y = |\lambda| \norm{T}$
		
		\item Seien $T_1, T_2 \in\BS$. Dann $\norm{T_1 + T_2} = \sup_{x \in \EK}(\norm{T_1x}_Y + \norm{T_2x}_Y)\leq \sup_{x_1,x_2 \in \EK}(\norm{T_1x}_Y + \norm{T_2x}_Y) = \sup_{x_1 \in \EK}\norm{T_1x_1}_Y + \sup_{x_2 \in \EK}\norm{T_2x_2}_Y= \norm{T_1} + \norm{T_2}$ %TODO hinkriegen, dass der Text wirklich unterm sup ist.
		\end{enumerate}
	\end{proof}
